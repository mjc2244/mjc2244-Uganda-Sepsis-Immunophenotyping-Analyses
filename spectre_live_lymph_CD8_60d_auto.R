...
#Set 'input' directory
setwd("C:\\Users\\mjcum\\OneDrive\\Desktop\\Live_lymph_pregate_CD8")
InputDirectory <- getwd()
setwd(PrimaryDirectory)

#Set 'metadata' directory
setwd(PrimaryDirectory)
setwd("C:\\Users\\mjcum\\OneDrive\\Desktop\\Live_lymph_pregate_CD8\\live_lymph_pregate_CD8_metadata")
MetaDirectory <- getwd()
setwd(PrimaryDirectory)
...
#Read in metadata
setwd(MetaDirectory)
meta.dat <- fread("pbmc_live_lymph_cd8_metadata.csv")
meta.dat
...
plot1 <- plot1 + scale_colour_manual(values = c("#E64B35FF", "#4DBBD5FF", "#00A087FF", "#3C5488FF", "#91D1C2FF", "#7E614EFF", "#8491B4FF", "#EFC000FF")) 
...
#Combine plots
library(ggpubr)
ggarrange(plot3, plot4, plot1, blankplot, hjust = c(-0.015,-0.03,-0.01,-0.01), ncol = 2, nrow = 2, labels = c("(a) Alive at 60-days", "(b) Dead at 60-days", "(c) FlowSOM metaclusters" , "(d) MFI by FlowSOM metacluster"))
...
tablecd8prop <- CreateTableOne(vars = listVars, 
                           data = sum.dat, 
                           factorVars = catVars,
                           strata = "Group_2",
                           includeNA = TRUE,
...
tablecd8prop <- print(tablecd8prop, nonnormal = c("Percent of sample -- 1",
                                                            "Percent of sample -- 2", 
                                                            "Percent of sample -- 3", 
                                                            "Percent of sample -- 4", 
                                                            "Percent of sample -- 5", 
...
library(tableHTML)
write_tableHTML(tableHTML(tablecd8prop), file = 'tablecd8prop60d.html')
...
setwd("C:\\Users\\mjcum\\OneDrive\\Desktop\\Live_lymph_pregate_CD8\\MEM")
getwd()
